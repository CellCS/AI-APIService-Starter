...
class LLMSService:
    ...
    def __init__(self):
        ...
        self.lmstudio_client =  OpenAI(base_url=appconsts.lmstudio_host_url, api_key=appconsts.lmstudio_apikey)
        ...
        self.llm_clients={appconsts.ai_clients[0]:self.ollama_client, 
                          appconsts.ai_clients[1]:self.lmstudio_client,
                          appconsts.ai_clients[2]:self.openai_client,
                          appconsts.ai_clients[3]:self.azureopenai_client,
                          }
    ...
    def generate_response(self, clientname, model, res):
        ...
        elif clientname == appconsts.ai_clients[1] or clientname == appconsts.ai_clients[2]:
                pprint.pprint(res)
                new_res = {
                    "client": clientname,
                    "model": model,
        ...
        elif clientname == appconsts.ai_clients[3]:
                pprint.pprint(res)
                new_res = {
                    "client": clientname,
                    "model": 'azure',
    ...
    def chat(self, client, model, messages,temperature=0.2):
        ...
        elif client == appconsts.ai_clients[1] or client == appconsts.ai_clients[2]:
            return self.v1chatcompletions(client, model, messages, temperature)
        elif client == appconsts.ai_clients[3]:
            return self.v1chatcompletions(client, model, messages, temperature)
    ...
    def generate(self, client, model, prompt, isstream, messages):
        ...
        if client == appconsts.ai_clients[0]:
            ...
            return self.generate_response(client, model, res)
        # only like OpenAI
    def v1chatcompletions(self, client, model, messages, temperature):
        if client not in appconsts.ai_clients or client == appconsts.ai_clients[0]:
            return {"messsages":[], "details": f"there is no {client}", "status_code":401}
        if client == appconsts.ai_clients[1]:
            completion = self.lmstudio_client.chat.completions.create(
                model=model,
                messages=messages,
